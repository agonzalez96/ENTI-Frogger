...
public:
	...
	void Rotate(Transform &transform,ObjectID objectID, int angle, SDL_RendererFlip flip) {
		auto center = SDL_Point{ transform.w - transform.w/2, transform.h - transform.h / 2 };
		SDL_RenderCopyEx(m_SDLRenderer, m_textureData[objectID], NULL, &transform.rect, angle, &center, flip);
	

	}

	std::unordered_map<ObjectID, SDL_Texture*> GetTextureData() {
